...
function getOceanPath(start, end) {
    ...
    // Check if we need to go around Africa
    if ((start.lng < 20 && end.lng > 70) || (start.lng > 70 && end.lng < 20)) {
        if (start.lat > 0 || end.lat > 0) {
            path.push({lat: 12, lng: 43}); // Bab el-Mandeb (entrance to Red Sea)
            path.push({lat: 30, lng: 32}); // Suez Canal
        } else {
            path.push({lat: -35, lng: 20}); // Cape of Good Hope
        }
    }
    // Check if we need to go around South America
    else if ((start.lng < -40 && end.lng > -70) || (start.lng > -70 && end.lng < -40)) {
        path.push({lat: -56, lng: -67}); // Cape Horn
    }
